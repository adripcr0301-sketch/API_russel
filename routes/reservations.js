...
router.get('/reservations', authenticate, async (req, res) => {
  try {
    ...
    if (isHtml(req)) return res.render('reservations/all', { reservations, user: req.user });
...
// réservations d'un catway spécifique
...
router.get('/catways/:id/reservations/:idReservation', authenticate, async (req, res) => {
  try {
    ...
    if (isHtml(req)) return res.render('reservations/detail', { reservation, user: req.user });
...
router.post('/catways/:id/reservations', authenticate, async (req, res) => {
  try {
    ...
    if (isHtml(req)) return res.redirect('/catways/' + req.params.id + '/reservations');
    res.status(201).json(reservation);
  } catch (err) {
    if (isHtml(req)) {
      return res.render('reservations/form', {
        reservation: req.body, catwayNumber: req.params.id, error: err.message, user: req.user
      });
    }
...
router.put('/catways/:id/reservations/:idReservation', authenticate, async (req, res) => {
  try {
    ...
    if (isHtml(req)) return res.redirect('/catways/' + req.params.id + '/reservations/' + req.params.idReservation);
    res.json(reservation);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
